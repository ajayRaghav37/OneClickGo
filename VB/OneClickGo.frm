...
Dim CurrentSong As Integer  'This variable is used to store playlist progressions i.e. -2, -1, 0, 1, 1, etc.
...
Private Sub Form_Load()
    ...
    mnuLinear.Enabled = Not mnuLinear.Checked
    mnuLinRev.Enabled = Not mnuLinRev.Checked
    mnuShuffle.Enabled = Not mnuShuffle.Checked
    ...
    mnuAutoBackup.Checked = GtSt("AutoBackup", False)
    ...
    If GtSt("UsedBefore", "0") = "0" Then
        ...
        Do
            ...
            If Not (ShellOpener Is Nothing) Then
                ...
                If TempStr <> GtSt("AllMusicFolder") And FileSystem.folderexists(TempStr) Then
                    Exit Do
                Else
                    If TempStr = GtSt("AllMusicFolder") Then
                        OldName = OldName & " [Cannot be same as All Music Folder]"
                    Else
                        OldName = OldName & " [Invalid Folder]"
                    End If
                End If
        ...
        If TempStr <> GtSt("MyMusicFolder", vbNullString) And GtSt("MyMusicFolder", vbNullString) <> vbNullString Then
            MyMsgBox = MsgBox("You are about to change your 'My Music' directory that might result in corruption of your playlist." & vbCrLf & "Are you sure you want to continue?", vbYesNo + vbCritical + vbSystemModal, "WARNING")
    ...
    FilNum = 1
    Do
        If GtSt(CStr(FilNum), vbNullString) <> vbNullString Then
            If FileSystem.FileExists(MyMusicFolder & "\" & GtSt(CStr(FilNum)) & ".mp3") Then
                MyMusicBox.AddItem GtSt(CStr(FilNum)) & ".mp3"
            ElseIf FileSystem.FileExists(AllMusicFolder & "\" & GtSt(CStr(FilNum)) & ".mp3") Then
                FileSystem.movefile AllMusicFolder & "\" & GtSt(CStr(FilNum)) & ".mp3", MyMusicFolder & "\" & GtSt(CStr(FilNum)) & ".mp3"
                MyMusicBox.AddItem GtSt(CStr(FilNum)) & ".mp3"
            Else
                MyMsgBox = MsgBox("This song has been deleted or moved to another location:" & vbCrLf & GtSt(CStr(FilNum)), vbCritical, "Song Not Found")
            End If
        Else
            Exit Do
        End If
    ...
    AllMusicBoxLabel.Caption = "  All Music (" & CStr(AllMusicBox.ListIndex + 1) & "/" & CStr(AllMusicBox.ListCount) & ")"
    MyMusicBoxLabel.Caption = "  My Music (" & CStr(MyMusicBox.ListIndex + 1) & "/" & CStr(MyMusicBox.ListCount) & ")"
    If Not mnuStart.Checked Then
        ...
        If TempNum > 0 Then
            ...
            Else
                TempStr = "songs"
            End If
            RecentAction.Caption = RecentAction.Caption & " (" & CStr(TempNum) & " new " & TempStr & " added to My Music)"
        End If
        Show
    Else
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    ...
    For FilNum = 1 To MyMusicBox.ListCount
        If MyMusicBox.List(FilNum - 1) <> GtSt(CStr(FilNum)) Then
            MadeChanges = True
            Exit For
        End If
    Next
    If GtSt(CStr(MyMusicBox.ListCount + 1)) <> vbNullString Then
        MadeChanges = True
    End If
...
Private Sub Form_Resize()
    ...
    If WindowState <> 1 Then
        ...
        If mnuMusicLeft.Checked Then
            MyMusicBox.Left = 120
            AllMusicBox.Left = AllMusicBox.Width + 240
        Else
            MyMusicBox.Left = MyMusicBox.Width + 240
            AllMusicBox.Left = 120
        End If
        If ColorSchemeBox.Visible Then
            AllMusicBox.Height = ScaleHeight - 960 - ColorSchemeBox.Height - 120
            ColorSchemeBox.Move AllMusicBox.Left, AllMusicBox.Top + AllMusicBox.Height + 120, AllMusicBox.Width
        Else
            AllMusicBox.Height = ScaleHeight - 960
        End If
        MyMusicBox.Height = ScaleHeight - 960
        SearchAll.Move AllMusicBox.Left + (AllMusicBox.Width / 2), SearchAll.Top, AllMusicBox.Width / 2
        AllSearch.Move SearchAll.Left, AllSearch.Top, SearchAll.Width
        MySearch.Move MyMusicBox.Left + (MyMusicBox.Width / 2), AllSearch.Top, SearchAll.Width
        SearchMy.Move MySearch.Left, SearchMy.Top, SearchAll.Width
        AllMusicBoxLabel.Move AllMusicBox.Left, AllMusicBoxLabel.Top, AllMusicBox.Width
        MyMusicBoxLabel.Move MyMusicBox.Left, MyMusicBoxLabel.Top, MyMusicBox.Width
        StatusBar.Move StatusBar.Left, MyMusicBox.Top + MyMusicBox.Height + 60, ScaleWidth - 240
        RecentAction.Move RecentAction.Left, StatusBar.Top + 75, StatusBar.Width - 630
        SeekFX.Move SeekFX.Left, StatusBar.Top, StatusBar.Width
        SeekBack.Top = StatusBar.Top
        CancelBtn.Move StatusBar.Left + StatusBar.Width - CancelBtn.Width - 30, StatusBar.Top + 45
        HideBtn.Move CancelBtn.Left - HideBtn.Width, CancelBtn.Top
        CCS3.Left = 120 + ColorSchemeBox.Width - 1500
        CCS2.Left = CCS3.Left - 1200
        For FilNum = 0 To 7
            If FilNum Mod 2 = 0 Then
                CCSC(FilNum).Left = CCS2.Left + (CCS2.Width - CCSC(FilNum).Width) / 2
            Else
                CCSC(FilNum).Left = CCS3.Left + (CCS3.Width - CCSC(FilNum).Width) / 2
            End If
        Next
    End If
End Sub
...
Private Sub CCSC_Click(Index As Integer) 'Changing the visual interface as per user's choice
    ...
    Select Case Index
        ...
        Case 6
            ...
            SeekBack.BackColor = RGB(MakeDiff(cRed), MakeDiff(cGreen), MakeDiff(cBlue))
    ...
    SvSt "Color" & CStr(Index), CStr(NewColor)
    Exit Sub
GetOut:
    Exit Sub
End Sub
...
Private Sub CCSF_Click(Index As Integer)
    ...
    SvSt "FN" & CStr(Index), NFN
    SvSt "FB" & CStr(Index), NFB
    SvSt "FI" & CStr(Index), NFI
    SvSt "FS" & CStr(Index), NFS
    Exit Sub
GetOut:
    Exit Sub
End Sub
...
Private Sub MainTimer_Timer() 'The main Timer of OCG that performs many tasks simultaneously
    ...
    If Not mnuSkinMode.Checked And Not mnuGaming.Checked Then
        ...
        If AllMusicBox.ListIndex = -1 And MyMusicBox.ListIndex = -1 Then
            mnuTransferNow.Enabled = False
            mnuDeleteFile.Enabled = False
            mnuRenameFile.Enabled = False
        Else
            mnuTransferNow.Enabled = True
            mnuDeleteFile.Enabled = True
            mnuRenameFile.Enabled = True
        End If
        mnuHit.Enabled = CBool(MyMusicBox.ListCount)
        mnuSend.Enabled = CBool(MyMusicBox.ListCount)
        mnuSave.Enabled = CBool(MyMusicBox.ListCount)
        If MyPlayer(0).URL = vbNullString And MyPlayer(1).URL = vbNullString Then
            mnuDND.Enabled = False
            mnuGaming.Enabled = False
        Else
            mnuDND.Enabled = True
            mnuGaming.Enabled = True
        End If
        If MyPlayer(ActiveIndex).URL = vbNullString Then
            mnuMedia.Enabled = False
            mnuSkinMode.Enabled = False
        Else
            mnuMedia.Enabled = True
            mnuSkinMode.Enabled = True
        End If
        
        If MyMusicBox.ListIndex = -1 Or TransferFrom = 0 Then
            mnuRating.Enabled = False
        Else
            mnuRating.Enabled = True
        End If
    End If
End Sub
...
Private Sub mnuAutoBackup_Click()
    mnuAutoBackup.Checked = Not mnuAutoBackup.Checked
    SvSt "AutoBackup", mnuAutoBackup.Checked
End Sub
...
Private Sub mnuBackup_Click() 'BackUp Menu
    Select Case BackUpSettings(InputBox("Where do you want OCG to create back up? OCG will create a file BackUp.ocgb in this location and overwrite any existing file.", Replace(mnuBackup.Caption, "&", vbNullString), GtSt("BackUpDir", App.Path)))
        Case 1
...
Private Function BackUpSettings(Optional BackupLocation As String = "") As Integer
    BackUpSettings = 0
    If FileSystem.folderexists(BackupLocation) Then
        SvSt "BackUpDir", BackupLocation
        BackupLocation = "REG EXPORT " & Chr$(34) & "HKCU\Software\VB and VBA Program Settings\ANIco.in\OneClick Go!" & Chr$(34) & " " & Chr$(34) & BackupLocation & "\BackUp.ocgb" & Chr$(34) & " /y"
        Shell BackupLocation, vbHide
        BackUpSettings = 1
    ElseIf BackupLocation = vbNullString Then
        BackUpSettings = 2
    End If
End Function
...
Private Sub mnuPSfaster_Click()
    If PlaySpd < 800 Then
        PlaySpd = PlaySpd + 5
    End If
    mnuPlaySpeed.Caption = "Pl&ay Speed (" & CStr(PlaySpd) & "%)"
End Sub
...
Private Sub mnuRestore_Click() 'Restore Menu
    TempStr2 = InputBox("Where did you save OCG back up file BackUp.ocgb? OCG will restore its playlist and settings from that file and overwrite any existing setting and playlist. The program will restart automatically.", Replace(mnuRestore.Caption, "&", vbNullString), GtSt("BackUpDir", App.Path))
...
Private Sub SeekFX_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If MyPlayer(ActiveIndex).URL <> vbNullString Then
        SeekFX.ToolTipText = DurationStr(X * MyPlayer(ActiveIndex).currentMedia.duration / SeekFX.Width) & "/" & DurationStr(MyPlayer(ActiveIndex).currentMedia.duration)
    End If
End Sub
...
Private Sub mnuBriefHistory_Click() 'Breif History menu
    ...
    If BriefHistory <> vbNullString Then
        MyMsgBox = MsgBox("BRIEF HISTORY" & vbCrLf & String(30, "—") & vbCrLf & vbCrLf & BriefHistory, vbInformation, "Brief History")
    Else
...
Private Sub mnuDeleteFile_Click() 'Delete Song menu
    If TransferFrom = 1 Then
        ...
        If MyMsgBox = vbNo Then
            Exit Sub
        End If
        If MyPlayer(ActiveIndex).URL = MyMusicFolder & "\" & LTrim$(OldName) & ".mp3" Then
            MyPlayer(ActiveIndex).URL = vbNullString
        End If
    ...
    Else
        ...
        If MyMsgBox = vbNo Then
            Exit Sub
        End If
        If MyPlayer(ActiveIndex).URL = AllMusicFolder & "\" & LTrim$(OldName) & ".mp3" Then
            MyPlayer(ActiveIndex).URL = vbNullString
        End If
...
Private Sub mnuDND_Click() 'Do Not Disturb menu
    mnuDND.Checked = Not mnuDND.Checked
    RecentAction.Caption = "Do Not Disturb mode turned "
    If mnuDND.Checked Then
        RecentAction.Caption = RecentAction.Caption & "on"
    Else
        RecentAction.Caption = RecentAction.Caption & "off"
    End If
End Sub
...
Private Sub mnuDown_Click() 'Move Down(Decrease Rating) menu
    ...
    End If
    mnuUp.Enabled = True
    If MyMusicBox.ListIndex = -1 Or TransferFrom = 0 Or MyMusicBox.ListIndex = MyMusicBox.ListCount - 1 Then
        mnuDown.Enabled = False
    Else
...
Private Sub mnuHit_Click() 'Copy Music Chart menu
    Dim HitList As Long
    If MyMusicBox.ListCount < 10 Then
        HitList = Val(InputBox("How many songs are to be selected for your Music Chart?", "Share Music Chart", LTrim$(MyMusicBox.ListCount)))
    Else
        HitList = Val(InputBox("How many songs are to be selected for your Music Chart?", "Share Music Chart", "10"))
    End If
    If HitList > MyMusicBox.ListCount Then
        HitList = MyMusicBox.ListCount
    ElseIf HitList <= 0 Then
        RecentAction.Caption = "Copying Music Chart cancelled by user or user made an invalid entry"
        Exit Sub
    End If
    OldName = vbNullString
    Clipboard.Clear
    For FilNum = 0 To HitList - 1
        OldName = OldName & LeadZero(FilNum + 1, Len(CStr(HitList))) & " | " & MyMusicBox.List(FilNum) & vbCrLf
    Next
...
Private Sub mnuMusicLeft_Click() 'Show My Music on Left menu
    mnuMusicLeft.Checked = Not mnuMusicLeft.Checked
...
Public Sub mnuNextSong_Click() 'The algorithm for Next Song is quite mathematical and logical
    ...
    If Not mnuRepeat.Checked Then
        If mnuPrevSong.Checked Then
            CurrentSong = CurrentSong - CInt(CFtimer.Enabled) - 1
        Else
    ...
    If Not AutoRepeated Then
        If GtSt2(CStr(CurrentSong), vbNullString) = vbNullString Then
            Do  'Looping if currentsong = nextsong!! Done just to avoid replaying of a song unintentionally
                If mnuShuffle.Checked Then
                    If PlayingAll Then
                        NowPlaying = Round(Rnd * (AllMusicBox.ListCount - 1), 0)
                    Else
                        NowPlaying = Round(Rnd * (MyMusicBox.ListCount - 1), 0)
                    End If
                Else
                    If (mnuLinear.Checked And Not mnuPrevSong.Checked) Or (mnuLinRev.Checked And mnuPrevSong.Checked) Then
                        If PlayingAll Then
                            NowPlaying = (NowPlaying + 1) Mod AllMusicBox.ListCount
                        Else
                            NowPlaying = (NowPlaying + 1) Mod MyMusicBox.ListCount
                        End If
                    Else
                        If PlayingAll Then
                            NowPlaying = (AllMusicBox.ListCount - 1) - ((AllMusicBox.ListCount - NowPlaying) Mod AllMusicBox.ListCount)
                        Else
                            NowPlaying = (MyMusicBox.ListCount - 1) - ((MyMusicBox.ListCount - NowPlaying) Mod MyMusicBox.ListCount)
                        End If
                    End If
                End If
                'Terminating conditions for the loop
                If PlayingAll Then
                    If AllMusicBox.ListCount = 1 Then
                        Exit Do
                    End If
                Else
                    If MyMusicBox.ListCount = 1 Then
                        Exit Do
                    End If
                End If
                If mnuPrevSong.Checked Then
                    If NowPlaying <> Val(GtSt2(CStr(CurrentSong + 1))) Then
                        Exit Do
                    End If
                Else
                    If NowPlaying <> Val(GtSt2(CStr(CurrentSong - 1))) Then
                ...
                End If
            Loop
            SvSt2 CStr(CurrentSong), CStr(NowPlaying)
        Else
            NowPlaying = Val(GtSt2(CStr(CurrentSong)))
        End If
    Else
        AutoRepeated = False
    End If
    If PlayingAll Then
        If CFtimer.Enabled Then
            MyPlayer(PassiveIndex).URL = AllMusicFolder & "\" & AllMusicBox.List(NowPlaying) & ".mp3"
        Else
            MyPlayer(ActiveIndex).URL = AllMusicFolder & "\" & AllMusicBox.List(NowPlaying) & ".mp3"
        End If
    Else
        If CFtimer.Enabled Then
            MyPlayer(PassiveIndex).URL = MyMusicFolder & "\" & MyMusicBox.List(NowPlaying) & ".mp3"
        Else
            MyPlayer(ActiveIndex).URL = MyMusicFolder & "\" & MyMusicBox.List(NowPlaying) & ".mp3"
        End If
    End If
...
Private Sub mnuPlay_Click() 'Play menu
    If MyPlayer(ActiveIndex).URL <> vbNullString Or MyPlayer(PassiveIndex).URL <> vbNullString Then
        ...
        Else
            ...
            Else
                If PlayingAll Then
                    RecentAction.Caption = "Media Paused: " & AllMusicBox.List(NowPlaying)
                Else
                    RecentAction.Caption = "Media Paused: " & MyMusicBox.List(NowPlaying)
                End If
            End If
        End If
    End If
...
Private Sub mnuRating_Click() 'Custom Rank menu
    FilNum = Val(InputBox("Enter the desired rank of this song", "OneClick Go!", CStr(MyMusicBox.ListIndex + 1)))
...
Private Sub mnuRenameFile_Click() 'Rename song menu
    If TransferFrom = 1 Then
        ...
        If FileSystem.FileExists(MyMusicFolder & "\" & NewName & ".mp3") Then
            ...
            Do
                TempNum = TempNum + 1
            Loop While FileSystem.FileExists(MyMusicFolder & "\" & NewName & CStr(TempNum) & ".mp3")
            NewName = NewName & CStr(TempNum)
        End If
    ...
    Else
        ...
        If FileSystem.FileExists(AllMusicFolder & "\" & NewName & ".mp3") Then
            ...
            Do
                TempNum = TempNum + 1
            Loop While FileSystem.FileExists(AllMusicFolder & "\" & NewName & CStr(TempNum) & ".mp3")
            NewName = NewName & CStr(TempNum)
        End If
...
Private Sub mnuCrossfade_Click() 'Crossfade menu
    mnuCrossfade.Checked = Not mnuCrossfade.Checked
...
Private Sub mnuSave_Click() 'Save playlist menu
    For FilNum = 1 To MyMusicBox.ListCount
        SvSt CStr(FilNum), MyMusicBox.List(FilNum - 1)
    Next
    FilNum = MyMusicBox.ListCount + 1
    Do
        If GtSt(CStr(FilNum)) <> vbNullString Then
            DlSt CStr(FilNum)
        Else
            Exit Do
        End If
    ...
    If mnuAutoBackup.Checked Then
        Select Case BackUpSettings(GtSt("BackUpDir", ""))
            Case 1
                RecentAction.Caption = "My Music Saved and Backed Up at " & CStr(Now)
                BriefHistory = Chr$(187) & " " & CStr(Now) & vbCrLf & RecentAction.Caption & vbCrLf & vbCrLf & BriefHistory
            Case Else
                RecentAction.Caption = "My Music Saved but Backup failed at " & CStr(Now)
                BriefHistory = Chr$(187) & " " & CStr(Now) & vbCrLf & RecentAction.Caption & vbCrLf & vbCrLf & BriefHistory
        End Select
    Else
        RecentAction.Caption = "My Music Saved at " & CStr(Now)
        BriefHistory = Chr$(187) & " " & CStr(Now) & vbCrLf & RecentAction.Caption & vbCrLf & vbCrLf & BriefHistory
    End If
End Sub
...
Private Sub mnuStart_Click() 'Auto-Start menu
    ...
    If mnuStart.Checked Then
        RecentAction.Caption = "Auto Start enabled"
    Else
        RecentAction.Caption = "Auto Start disabled"
    End If
...
Private Sub mnuStop_Click() 'Stop menu
    If MyPlayer(ActiveIndex).URL <> vbNullString Then
        ...
        If Not mnuGaming.Checked Then
            SeekBack.Width = 0
            If PlayingAll Then
                RecentAction.Caption = "Media Stopped: " & AllMusicBox.List(NowPlaying)
            Else
                RecentAction.Caption = "Media Stopped: " & MyMusicBox.List(NowPlaying)
            End If
        End If
    End If
End Sub
...
Private Sub mnuUp_Click() 'Move Up(Increase Rating) menu
    ...
    End If
    mnuDown.Enabled = True
    If MyMusicBox.ListIndex = -1 Or TransferFrom = 0 Or MyMusicBox.ListIndex = 0 Then
        mnuUp.Enabled = False
    Else
        mnuUp.Enabled = True
    End If
...
Private Sub MyBoxesClick(BoxName As ListBox) 'Clicking on a music box calls MyBoxesClick
    ...
    Else
        TransferFrom = 1
        mnuTransferNow.Caption = "&Move to All Music" & Chr$(9) & "Space"
    End If
    If Not IsContinue And Not mnuDND.Checked Then
        SvSt2 "JustToClear", vbNullString
        DeleteSetting "ANIco.in", "SongCache"
        CurrentSong = 0
        SvSt2 CStr(CurrentSong), CStr(BoxName.ListIndex)
        If Not CFtimer.Enabled Then
            ActiveCS = CurrentSong
        End If
        ...
        If BoxName.Name = "AllMusicBox" Then
            If CFtimer.Enabled Then
                MyPlayer(PassiveIndex).URL = AllMusicFolder & "\" & AllMusicBox.List(AllMusicBox.ListIndex) & ".mp3"
            Else
                MyPlayer(ActiveIndex).URL = AllMusicFolder & "\" & AllMusicBox.List(AllMusicBox.ListIndex) & ".mp3"
            End If
            PlayingAll = True
        Else
            If CFtimer.Enabled Then
                MyPlayer(PassiveIndex).URL = MyMusicFolder & "\" & MyMusicBox.List(MyMusicBox.ListIndex) & ".mp3"
            Else
                MyPlayer(ActiveIndex).URL = MyMusicFolder & "\" & MyMusicBox.List(MyMusicBox.ListIndex) & ".mp3"
            End If
            PlayingAll = False
        End If
        mnuPlay.Caption = "&Pause"
...
Private Sub MyBoxesDblClick(BoxName As ListBox) 'Double Clicking on a music box calls MyBoxesDblClick
    ...
    If BoxName.ListIndex <> -1 Then
        If BoxName.Name = "AllMusicBox" Then
            TempStr = InputBox("Enter the new rank of this song", "Adding New Song", CStr(MyMusicBox.ListCount + 1))
            If TempStr <> vbNullString And Val(TempStr) > 0 And Val(TempStr) <= MyMusicBox.ListCount + 1 And Val(TempStr) - Int(Val(TempStr)) = 0 Then
                ...
                Else
                    ...
                    Do
                        TempN2 = TempN2 + 1
                    Loop While SendMessage(MyMusicBox.hWnd, &H18F, -1, ByVal AllMusicBox.List(AllMusicBox.ListIndex) & CStr(TempN2)) <> -1
                    MyMusicBox.AddItem AllMusicBox.List(AllMusicBox.ListIndex) & CStr(TempN2), Val(TempStr) - 1
                End If
            Else
            ...
            If TempN2 Then
                OldName = "Song selected for My Music and renamed: "
            Else
                OldName = "Song selected for My Music: "
            End If
        Else
            ...
            Else
                ...
                Do
                    TempN2 = TempN2 + 1
                Loop While SendMessage(AllMusicBox.hWnd, &H18F, -1, ByVal MyMusicBox.List(MyMusicBox.ListIndex) & CStr(TempN2)) <> -1
                AllMusicBox.AddItem MyMusicBox.List(MyMusicBox.ListIndex) & CStr(TempN2)
                FileSystem.movefile MyMusicFolder & "\" & MyMusicBox.List(MyMusicBox.ListIndex) & ".mp3", AllMusicFolder & "\" & MyMusicBox.List(MyMusicBox.ListIndex) & CStr(TempN2) & ".mp3"
            End If
            If TempN2 Then
                OldName = "Song removed from My Music and renamed: "
            Else
                OldName = "Song removed from My Music: "
            End If
        End If
...
Private Sub SongCacheReset()
    ...
    End If
    If PlayingAll Then
        NowPlaying = AllMusicBox.ListIndex
    Else
        NowPlaying = MyMusicBox.ListIndex
    End If
    SvSt2 CStr(CurrentSong), CStr(NowPlaying)
End Sub
