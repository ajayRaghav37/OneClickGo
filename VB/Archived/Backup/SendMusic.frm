...
Private Sub Form_Load()
    ...
    ISentIt = True
    MyCheck_Click
    AllMusicBoxLabel.Caption = "     All Music (" & Trim$(Str$(AllMusicBox.SelCount) & "/" & Trim$(Str$(AllMusicBox.ListCount)) & ")")
    MyMusicBoxLabel.Caption = "     My Music (" & Trim$(Str$(MyMusicBox.SelCount) & "/" & Trim$(Str$(MyMusicBox.ListCount)) & ")")
...
Private Sub AllCheck_Click() 'Selecting/Deselecting all songs in AllMusicBox
    If ISentIt Then
        ...
        For FilNum = 0 To AllMusicBox.ListCount - 1
            AllMusicBox.Selected(FilNum) = IIf(AllCheck.Value = 1, True, False)
        Next
...
Private Sub AllMusicBoxLabel_Click() 'Checking/Unchecking AllCheck checkbox through label
    AllCheck.Value = IIf(AllCheck.Value = 0, 1, 0)
    AllCheck_Click
End Sub
        ...
        If Not (ShellOpener Is Nothing) Then
            ...
            If FileSystem.folderexists(DeviceFolder) Then
                ...
                If RemCheck.DriveType = 2 Then
                    MyMsgBox = MsgBox("The device you selected is fixed to your computer. It can cause redundancy in your files." & Chr$(13) & "Are you sure you want to send music?", vbExclamation + vbYesNo + vbDefaultButton2, "Fixed drive selected")
...
Private Sub SearchAll_Change() 'Search songs
    ...
    For FilNum = 0 To AllMusicBox.ListCount - 1
        If InStr(1, AllMusicBox.List(FilNum), SearchAll.Text, vbTextCompare) <> 0 Then
            AllSearch.AddItem AllMusicBox.List(FilNum) & " (" & LTrim$(Str$(FilNum + 1)) & ")"
        End If
    Next
...
Private Sub SearchMy_Change()
    ...
    For FilNum = 0 To MyMusicBox.ListCount - 1
        If InStr(1, MyMusicBox.List(FilNum), SearchMy.Text, vbTextCompare) <> 0 Then
            MySearch.AddItem MyMusicBox.List(FilNum) & " (" & LTrim$(Str$(FilNum + 1)) & ")"
        End If
    Next
...
'Things doesn't work out well when Sync is turned on
    ...
    If mnuSynchronize.Checked Then 'Deleting extra songs before sending is started
        ...
        For FilNum = 0 To MyMusicBox.ListCount - 1
            ...
            If MyMusicBox.Selected(FilNum) Then
                ...
                For TempN3 = 0 To Val(GtSt("MaxCount", LTrim$(Str$(MyMusicBox.ListCount))))
                    
                    'Example values stored in DestFile1
                    'If TempN3 = 0, then "
                    'Else,
                    DestFile1 = IIf(TempN3 = 0, vbNullString, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(TempN3))), "0") & LTrim$(Str$(TempN3)) & " ") & MyMusicBox.List(FilNum) & ".mp3"
        ...
        For FilNum = 0 To AllMusicBox.ListCount - 1
            If AllMusicBox.Selected(FilNum) Then
                ...
                For TempN3 = 0 To Val(GtSt("MaxCount", LTrim$(Str$(MyMusicBox.ListCount))))
                    DestFile1 = IIf(TempN3 = 0, vbNullString, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(TempN3))), "0") & LTrim$(Str$(TempN3)) & " ") & AllMusicBox.List(FilNum) & ".mp3"
                    DestFile = DeviceFolder & "\" & DestFile1
                    If FileSystem.FileExists(DestFile) Then
                        NewEntry = False
                        Exit For
    ...
    For FilNum = 0 To MyMusicBox.ListCount - 1
        If MyMusicBox.Selected(FilNum) Then 'Checking if it was selected before sending the song
            ...
            For TempN3 = 0 To Val(GtSt("MaxCount", LTrim$(Str$(MyMusicBox.ListCount))))
                DestFile = DeviceFolder & "\" & IIf(TempN3 = 0, vbNullString, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(TempN3))), "0") & LTrim$(Str$(TempN3)) & " ") & MyMusicBox.List(FilNum) & ".mp3"
                If FileSystem.FileExists(DestFile) Then
                    NewEntry = False
                    Exit For
                End If
            ...
            If NewEntry Then
                FileSystem.copyfile MyMusicFolder & "\" & MyMusicBox.List(FilNum) & ".mp3", DeviceFolder & "\" & IIf(mnuRankWrite.Checked, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(FilNum + 1))), "0") & LTrim$(Str$(FilNum + 1)) & " ", vbNullString) & MyMusicBox.List(FilNum) & ".mp3"
            Else
                Name DestFile As DeviceFolder & "\" & IIf(mnuRankWrite.Checked, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(FilNum + 1))), "0") & LTrim$(Str$(FilNum + 1)) & " ", vbNullString) & MyMusicBox.List(FilNum) & ".mp3"
            End If
        End If

        OneClickGo.RecentAction.Caption = "Sending your music to your device... " & Trim$(Str$(Round(TempNum * 100 / (AllMusicBox.SelCount + MyMusicBox.SelCount), 0))) & "%" & IIf(OneClickGo.CancelBtn.Visible, " (Click cancel to pause)", vbNullString)
        DoEvents
    Next
    For FilNum = 0 To AllMusicBox.ListCount - 1
        If AllMusicBox.Selected(FilNum) Then
            ...
            For TempN3 = 0 To Val(GtSt("MaxCount", LTrim$(Str$(MyMusicBox.ListCount))))
                DestFile = DeviceFolder & "\" & IIf(TempN3 = 0, vbNullString, String(Len(LTrim$(Str$(MyMusicBox.ListCount))) - Len(LTrim$(Str$(TempN3))), "0") & LTrim$(Str$(TempN3)) & " ") & AllMusicBox.List(FilNum) & ".mp3"
                If FileSystem.FileExists(DestFile) Then
                    NewEntry = False
                    Exit For
                End If
            ...
            If NewEntry Then
                FileSystem.copyfile AllMusicFolder & "\" & AllMusicBox.List(FilNum) & ".mp3", DeviceFolder & "\" & IIf(mnuRankWrite.Checked, LTrim$(Str$(MyMusicBox.ListCount + 1)) & " ", vbNullString) & AllMusicBox.List(FilNum) & ".mp3"
            Else
                Name DestFile As DeviceFolder & "\" & IIf(mnuRankWrite.Checked, LTrim$(Str$(MyMusicBox.ListCount + 1)) & " ", vbNullString) & AllMusicBox.List(FilNum) & ".mp3"
            End If
        End If
    ...
    BriefHistory = Chr$(187) & " " & Str$(Now) & Chr$(13) & OneClickGo.RecentAction.Caption & Chr$(13) & Chr$(13) & BriefHistory
    Unload Me
    Exit Sub
OnError:
